"""Streaming Gradio CATE explorer (local-dev twin of hf_space/app.py).
...
UX mirrors the P1 Jailbreak Detector portfolio app: tabs (Try It / How It
Works / Limitations), clickable persona examples, and a big colored
TREAT/DEFER decision card. Pipeline runs as a 4-stage streaming generator
that yields HTML progressively (rule C43).
"""
...
def stream_predict(  # noqa: PLR0913
    ...
    """Yield (pipeline_html, result_html) progressively across 4 stages."""
    steps = (
...
# Personas (4 named examples to drop the activation barrier)
EXAMPLES = [
    [28, 12, 1, 0, 0, 0, 0, 0],
    [22, 9, 1, 0, 0, 1, 0, 0],
...
- *"Of 1,000 applicants, who should we prioritize for our limited
  training slots?"*
- *"Are there profiles where training does not help, or even hurts?"*
...
controls were employed. They are not comparable.
...
- **TREAT** (green) - 95% range entirely above zero. Confident lift.
- **INSUFFICIENT EVIDENCE** (amber) - range crosses zero. Cannot tell.
- **DO NOT TREAT** (red) - range entirely below zero. Predicted harm.
"""

_LIMITS_MD = """
### When the model is wrong

- **Population shift**: NSW participants were US workers in the 1970s.
  Predictions for very different populations are extrapolation.
- **Unmeasured confounders**: motivation, family support, local labor
  conditions. The model assumes the 8 measured attributes capture
  everything that matters for selection.
- **Small sample**: 445 NSW participants. Estimates for unusual
  profiles (very old + very educated + high prior earnings) are
  imprecise.

### Intended use

Pedagogical / portfolio. This is **not** a deployed policy tool.
"""


def build_demo() -> gr.Blocks:
    """Construct the gr.Blocks UI - mirror of hf_space/app.py."""
...
def main() -> None:
    """Launch the demo with the dark Base theme + custom CSS."""
